...
function SuiteOrchestration() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (!over || !selectedSuiteId) return

    const activeId = active.id as string
    const overId = over.id as string

    const isInSuite = selectedSuite?.caseIds.includes(activeId)
    const isTargetInSuite = selectedSuite?.caseIds.includes(overId)

    if (isInSuite && isTargetInSuite) {
      const oldIndex = selectedSuite!.caseIds.indexOf(activeId)
      ...
      const newCaseIds = [...selectedSuite!.caseIds]
      const insertIndex = newCaseIds.indexOf(overId)
      newCaseIds.splice(insertIndex, 0, activeId)
      reorderSuiteCases(selectedSuiteId, newCaseIds)
  ...
  const handleDeleteSuite = (id: string) => {
    ...
    if (selectedSuiteId === id) {
      setSelectedSuiteId(suites.length > 1 ? suites.find((s) => s.id !== id)?.id || null : null)
    }
                ...
                renderItem={(suite) => (
                  <List.Item
                    ...
                    style={{
                      ...
                      marginBottom: 4,
                      border:
                        selectedSuiteId === suite.id ? '1px solid #1677ff' : '1px solid transparent',
                    }}
          ...
          <Card size="small" title="可用用例" style={{ height: '100%' }}>
            ...
            >
              {availableCases.length === 0 ? (
                ...
                >
                  <SortableContext items={availableCases.map((c) => c.id)} strategy={verticalListSortingStrategy}>
                  ...
                  <DragOverlay>
                    {activeCase ? <SortableCaseItem caseData={activeCase} /> : null}
                  </DragOverlay>
                </DndContext>
              )}
            </div>
          ...
          >
            {!selectedSuite ? (
              ...
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                ...
                >
                  <div className="empty-suite-tip">
                    <Text type="secondary">拖拽用例到此处添加到套件</Text>
              ...
              <DndContext
                ...
                onDragEnd={handleDragEnd}
              >
                <SortableContext items={suiteCases.map((c) => c.id)} strategy={verticalListSortingStrategy}>
                  {suiteCases.map((caseItem, index) => (
                    <SortableCaseItem
                      key={caseItem.id}
                      caseData={caseItem}
                      showRemove
